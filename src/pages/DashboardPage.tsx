...
import { Gauge, LayoutGrid, Loader2, Tags } from "lucide-react";
import AppCard from "@/components/AppCard";
import { healthScoreTier } from "@/lib/health";
import type { StorePlatform } from "@/lib/stores/types";

/** Shape returned by GET /api/apps — identical to the old server component's
 * Prisma query (JSON-serialized: dates arrive as ISO strings). */
interface DashboardApp {
  ...
  pinned: boolean;
  _count: { keywords: number; competitors: number };
  healthReports: { score: number }[];
}
...
export default function DashboardPage() {
  ...
  const loadApps = useCallback(async () => {
    try {
      ...
      const data: { apps: DashboardApp[] } = await res.json();
      setApps(data.apps);
      setError(null);
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      {error && <div className="text-sm text-red-500 animate-fade-in-up mb-4">{error}</div>}
      ...
      {apps.length === 0 ? (
        <div className="animate-fade-in-up flex flex-col items-center gap-3 rounded-xl border border-dashed border-border p-12 text-center text-muted [animation-delay:80ms]">
          ...
          <div>
            No apps yet.{" "}
            <Link to="/apps/new" className="text-accent font-medium hover:underline">
              Add your first app
            </Link>{" "}
